...
return order;}
function buildGrid(){gridEl.innerHTML="";state.tiles=[];for(var i=0;i<TILE_COUNT;i+=1){var tile=document.createElement("button");tile.type="button";tile.className="tile-7520cae1";tile.disabled=true;tile.setAttribute("aria-label","Tile "+(i+1));tile.innerHTML='<span class="tile-inner-7520cae1">'+'<span class="tile-face-7520cae1 tile-front-7520cae1"></span>'+'<span class="tile-face-7520cae1 tile-back-7520cae1"></span>'+"</span>";tile.addEventListener("click",makeTileHandler(i));gridEl.appendChild(tile);state.tiles.push(tile);}}
function makeTileHandler(index){return function(){onTilePick(index);};}
function revealTile(index,isMine,dim){var tile=state.tiles[index];if(!tile||tile.classList.contains("revealed-7520cae1")){return;}
var back=tile.querySelector(".tile-back-7520cae1");if(back){back.innerHTML=isMine?BOMB_SVG:GEM_SVG;}
tile.classList.add("revealed-7520cae1");if(isMine){tile.classList.add("boom-7520cae1");}
if(dim){tile.classList.add("dimmed-7520cae1");}
tile.disabled=true;}
function revealRemaining(dimAll){for(var i=0;i<TILE_COUNT;i+=1){if(!state.tiles[i].classList.contains("revealed-7520cae1")){revealTile(i,state.mineMap[i]===true,dimAll);}}}
function setTilesEnabled(enabled){for(var i=0;i<TILE_COUNT;i+=1){var tile=state.tiles[i];tile.disabled=!enabled||tile.classList.contains("revealed-7520cae1");}}
...
var tile=state.tiles[index];if(!tile||tile.classList.contains("revealed-7520cae1")){return;}
...
function bust(){var lost=state.bet;revealRemaining(true);setStatus("Boom! The mine took your "+formatMoney(lost)+" bet.","lose");if(panelEl){panelEl.classList.remove("jolt-7520cae1");void panelEl.offsetWidth;panelEl.classList.add("jolt-7520cae1");}
endRound();}
